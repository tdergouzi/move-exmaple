...
module BasicCoin::BasicCoin {
    ...
    spec publish_balance {
        include SchemaPublish<CoinType> {addr: Signer::address_of(account), amount: 0};   
    }

    spec schema SchemaPublish<CoinType> {
        addr: address;
        amount: u64;
    ...
    spec mint {
        include SchemaDeposit<CoinType> {addr: mint_addr, amount};
    }
    ...
    spec withdraw {
        ...
        ensures balance_post == balnace - amount;
    }
    ...
    spec schema SchemaDeposit<CoinType> {
        addr: address;
        amount: u64;
        let balance = global<Balance<CoinType>>(addr).coin.value;
        aborts_if !exists<Balance<CoinType>>(addr);
